...
def test_critic_value_invalid_alpha_low():
    ...
    with pytest.raises(ValueError):
        f.critic_value(-0.25, "left")

def test_critic_value_invalid_alpha_high():
    f = FSnedecor(10, 12)
    with pytest.raises(ValueError):
        f.critic_value(1.07, "left")
...
def test_multiple_calls_consistent():
    f = FSnedecor(10, 12)
    left = f.critic_value(0.01, "left")
    right = f.critic_value(0.01, "right")
    assert right > left

def test_invalid_tail_in_critic_value():
    f = FSnedecor(10, 12)
    with pytest.raises(ValueError):
        f.critic_value(0.02, "invalid")
