...
def test_critic_value_right_tail():
    chisq = ChiSquare(3)
    result = chisq.critical_value(0.05, tail="right")
    expected = chi2.ppf(0.95, 3)
    assert pytest.approx(result) == expected

def test_critic_value_left_tail():
    chisq = ChiSquare(3)
    result = chisq.critical_value(0.05, tail="left")
...
def test_critic_value_invalid_alpha_low():
    ...
    with pytest.raises(ValueError):
        chisq.critical_value(-0.1, tail="right")

def test_critic_value_invalid_alpha_high():
    chisq = ChiSquare(3)
    with pytest.raises(ValueError):
        chisq.critical_value(1.5, tail="left")
...
def test_multiple_calls_consistent():
    chisq = ChiSquare(2)
    alpha = 0.05
    right = chisq.critical_value(alpha, tail="right")
    left = chisq.critical_value(alpha, tail="left")
    assert right > left

def test_invalid_tail_in_critic_value():
    chisq = ChiSquare(3)
    with pytest.raises(ValueError):
        chisq.critical_value(0.05, tail="invalid")
